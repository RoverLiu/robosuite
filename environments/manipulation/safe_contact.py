...
# Default safe contact environment configuration
DEFAULT_SAFE_CONTACT_CONFIG = {
    # settings for reward
    ...
    # setting for object
    "object_dispose_position": (0.1, -0.25, 0.8),
...
class SafeContact(SingleArmEnv):
    """
    ...
    def __init__(
        ...
        # Get config
        self.task_config = task_config if task_config is not None else DEFAULT_SAFE_CONTACT_CONFIG
        ...
        self.object_dispose_position = np.array(self.task_config["object_dispose_position"]
)
...
# ########################################################################################################################
            - SafeContact: object to some position with some requirements
                contact condition with object
                distance between object and target position

            - Large Acceleration Penalty: in [-inf, 0], scaled by estimated wiper acceleration and directly
              proportional to self.ee_accel_penalty

            - Reaching: in [0, 0.1], proportional to the distance between the gripper and the end position
